...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 1900, 800)
    canvas_nuis.SetBottomMargin(0.20)
    canvas_nuis.SetLeftMargin(0.05)
    canvas_nuis.SetRightMargin(0.05)    
    ...
    leg.Draw()
    canvas_nuis.SaveAs(options.plotfile.replace('.root','.png'))
    fout.WriteTObject(canvas_nuis)

    canvas_prop = ROOT.TCanvas("nuisances_bbb", "nuisances_bbb", 1900, 800)
    ...
    canvas_nuis.RedrawAxis()
    canvas_nuis.RedrawAxis('g')
    leg2=ROOT.TLegend(0.8,0.8,0.99,0.99)
    leg2.SetFillColor(0)
    leg2.SetTextFont(42)
    leg2.AddEntry(hist_prefit,"Prefit","FL")
    leg2.AddEntry(gr_fit_b,"B-only fit","EPL")
    leg2.AddEntry(gr_fit_s,"S+B fit"   ,"EPL")
    leg2.Draw()
    canvas_prop.SaveAs(options.plotfile.replace('.root','_bbb.png'))
    fout.WriteTObject(canvas_prop)
